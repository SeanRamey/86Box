...
static void rebuild_axis_button_selections(WindowHandle hdlg)
{
...
static int get_axis(WindowHandle hdlg, int id)
{
...
static int get_pov(WindowHandle hdlg, int id)
{
        WindowHandle h = GetDlgItem(hdlg, id);
        int axis_sel = SendMessage(h, CB_GETCURSEL, 0, 0);
...
#endif
joystickconfig_dlgproc(WindowHandle hdlg, UINT message, WPARAM wParam, LPARAM lParam)
{
	WindowHandle h;
	int c;
	int id;
...
uint8_t joystickconfig_open(WindowHandle hwnd, int joy_nr, int type)
{
