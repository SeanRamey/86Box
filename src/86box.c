...
void
emu_generate_tempfilename(char *buff, size_t bufsize, char *prefix, char *suffix)
{
	// get current time, convert to struct tm in localtime
	time_t currentTime = time(NULL);
	struct tm* my_time = localtime(&currentTime);

	// get current time in seconds and nanoseconds
	struct timespec ts;
	timespec_get(&ts, TIME_UTC);

	snprintf(buff, bufsize, "%s%04d-%02d-%02d-%02d:%02d:%02d-%03u%s",
		prefix ? prefix : "",
